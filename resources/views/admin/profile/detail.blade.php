...
<div class="container-fluid">
    ...
    <div class="card shadow-lg mb-4" style="border-radius: 15px;">
        <div class="card-header text-center" style="background: linear-gradient(to right, #007bff, #6c757d); color: white; border-radius: 15px 15px 0 0;">
            <h3 class="font-weight-bold">{{ $profile->item }}</h3>
            <p class="text-light" style="font-size: 1.2rem;">{{ \Carbon\Carbon::parse($profile->created_at)->isoFormat('D MMMM YYYY') }}</p>
        </div>
        ...
        <div class="card-footer text-center py-4" style="background-color: #f8f9fa; border-radius: 0 0 15px 15px;">
            <div class="row text-center">
                <div class="col">
                    <form action="{{ route('admin.profiles.edit') }}" method="post">
                        @csrf
                ...
                <div class="col">
                    <form action="{{route('admin.profiles.delete')}}" method="post">
                        @csrf
                        <input type="hidden" name="profile_id" value="{{$profile->profile_id}}">
